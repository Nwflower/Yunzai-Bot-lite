...
export default class onlineEvent extends EventListener {
  ...
  async execute (e) {
    logger.mark('----^_^----')
    logger.mark(logger.green(`Yunzai-Bot Lite 上线成功 版本v${cfg.package.version}`))
    logger.mark(logger.green('原版仓库https://github.com/Le-niao/Yunzai-Bot'))
    logger.mark(logger.green('轻量版仓库https://github.com/Nwflower/Yunzai-Bot-lite'))
    // logger.mark('-----------')
    /** 加载插件 */
